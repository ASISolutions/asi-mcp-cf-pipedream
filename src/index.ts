...
async function createConnectLink(
  ...
  // Per docs: append ?app={slug} to connect_link_url
...
async function deleteAccount(
  ...
  if (res.status !== 204) throw new Error(`Delete account failed ${res.status}`);
}
...
function sanitizeProxyHeaders(headers?: Record<string, string>): Record<string, string> | undefined {
...
async function proxyRequest(
  ...
  if (params.method && params.method !== "POST") headers["X-HTTP-Method-Override"] = params.method;
  ...
  async init() {
    ...
    this.server.tool(
      ...
      async ({ apps }: { apps?: ("hubspot" | "xero" | "pandadoc")[] }) => {
        ...
        const filter = (data: NonNullable<typeof res.data>) =>
          apps ? data.filter((a) => a.app?.name_slug && apps.includes(a.app.name_slug as "hubspot" | "xero" | "pandadoc")) : data;
        ...
        return {
          content: [
            { type: "text", text: JSON.stringify({ external_user_id, accounts: data }) },
          ],
        };
      }
    );
    ...
    this.server.tool(
      ...
      async ({ app }: { app: "hubspot" | "xero" | "pandadoc" }) => {
        const external_user_id = this.getExternalUserId();
        const pdToken = await getPdAccessToken(this.env);
        const url = await createConnectLink(this.env, pdToken, external_user_id, app);
        return {
          content: [
            {
              type: "text",
    ...
    this.server.tool(
      ...
      async ({ app, account_id }: { app: "hubspot" | "xero" | "pandadoc"; account_id?: string }) => {
        const external_user_id = this.getExternalUserId();
        const pdToken = await getPdAccessToken(this.env);
        ...
        if (!acctId) {
          return { content: [{ type: "text", text: `No ${app} account found.` }] };
        }
    ...
    this.server.tool(
      ...
      async ({ method, url, headers, body }: { 
        ...
        if (account && !account.credentials) {
          try {
            const detailed: any = await getAccountWithCredentials(this.env, pdToken, account.id);
            if (detailed?.data?.credentials) {
              account = { ...account, credentials: detailed.data.credentials } as any;
            }
          } catch {}
        }
        ...
        [...resp.headers.entries()].slice(0, 24).forEach(([k, v]) => (outHeaders[k] = v));
    ...
    this.server.tool(
      "proxy.request",
      {
        ...
        body: z.union([z.string(), z.record(z.any())]).optional(),
        account_id: z.string().optional(),
        app: z.enum(["hubspot", "xero", "pandadoc"]).optional(),
      },
      async ({ method, url, headers, body, account_id, app }: {
        method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE";
        url: string;
        headers?: Record<string, string>;
        body?: string | Record<string, unknown>;
        ...
        // Resolve account
        let acctId = account_id;
        const appFromUrl = /^https?:\/\//i.test(url) ? detectAppSlugFromUrl(url) : undefined;
        const resolvedApp = app || appFromUrl;
        if (!acctId) {
          const listed = await listAccountsForUser(
            this.env,
            pdToken,
        ...
        if (!acctId) {
          const connectUrl = await createConnectLink(this.env, pdToken, external_user_id, resolvedApp || "hubspot");
          return {
            content: [
              {
                type: "text",
        ...
        if (typeof body === "string") {
          try { proxyBody = JSON.parse(body); } catch { proxyBody = body; }
        }
